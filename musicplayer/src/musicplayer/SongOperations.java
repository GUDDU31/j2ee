...
public class SongOperations {
	...
	static ArrayList<Songs1> songsCollection = new ArrayList<Songs1>();
	static int size;
	static Songs1 song[] = new Songs1[size];
		...
		while (stop) {
			try {
				...
				for (int i = 0; i < size; i++) {
					...
					System.out.println();
					song[i] = new Songs1(i, songName, singerName, movieName, songDuration);
			...
			while (stop)
				try {
					...
					if (songNameNumber == 0) {
						...
						Songs1 songNameSongs = songsCollection.get(songNameNumber - 1);
